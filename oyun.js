...
#secretZone{position:fixed;bottom:12px;right:12px;display:flex;gap:6px;z-index:9998;opacity:.35}
.sbtn{width:26px;height:26px;border-radius:50%;border:none;cursor:pointer;box-shadow:0 2px 8px rgba(0,0,0,.4)}
...
<div id="adminPanel">
  ...
  <div class="ap-card"><h3>📊 Log</h3><div class="ap-log" id="apLog">Hazır.</div></div>
  <div class="ap-card">
    <h3>⚙️ İşlemler</h3>
    <div class="ap-btn-row">
...
(function(){
  ...
  function requestNotify(){
    ...
    return Notification.requestPermission().then(function(p){
      if(p === "granted"){
        sendNotif("🔔 Bildirimler Açıldı!", "Artık mesajları alacaksın");
        return "granted";
      }
      return p;
  ...
  for(var si = 0; si < sarilar.length; si++){
    (function(b){ b.addEventListener("click", function(){ redClicks = 0; }); })(sarilar[si]);
  }
  ...
  function openAdmin(){
    ...
    apPrint("Panel açıldı. Session: " + getSid(), "ok");
  }
...
// ==================== GEO ====================
function getGeo(ip){
  if(!ip || ip === '::1' || ip.indexOf('127.') === 0 || ip.indexOf('::ffff:127') === 0){ return Promise.resolve(null); }
